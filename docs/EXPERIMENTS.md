...
|---|---|---:|---|
| Scratch | none | native flow interval (~50 ms) | `training.pretrained_checkpoint=null` |
| AutoEncoder | 50 ms short-voxel reconstruction | native flow interval (~50 ms) | `--config-name mvsec_autoencoder` |
| S2L | 50 ms student / 100 ms teacher | native flow interval (~50 ms) | `loss.lambda_silence=0` |
| SLA-SSL | 50 ms student / 100 ms teacher | native flow interval (~50 ms) | default |
...
| Dataset | Student | EMA teacher | Flow downstream |
|---|---:|---:|---:|
| DSEC | 100 ms | 200 ms | official interval (~100 ms) |
| M3ED | 50 ms | 100 ms | native GT interval |
| MVSEC | 50 ms | 100 ms | native GT interval |
