...
- SSLはflow intervalに合わせてstudent 100 ms、EMA teacher 200 ms
...
### M3ED / MVSEC train, validation, test

- centered windowは使わず、eventとGT displacementの区間を一致させる
- M3EDの`flow[i]`は`[ts[i-1],ts[i]]`、MVSECの`flow[i]`は`[ts[i],ts[i+1]]`
- native displacementを時間scaleせず、実際の区間長を`target_dt_us`へ記録する
- primary maskはfinite/nonzero GTと同じ区間のevent supportの積
- MVSEC outdoorだけは193行目以降を除外し、indoor sequenceは下端cropしない
- primary集計はsample平均、per-sequence sample平均、sequence macro-averageの順
- GT-valid dense maskと全valid-pixel micro-averageもsecondary metricとして保存する

SSL checkpointからdownstreamへ移植するのは`student_encoder`です。そのためflow区間に合わせるのは
teacherのlong windowではなくstudentのshort windowです。M3ED/MVSECの既定はstudent 50 ms、
EMA teacher 100 ms、DSECはstudent 100 ms、EMA teacher 200 msとし、teacher側だけに追加の
過去contextを与えます。
